...
// 💡 타입 확장: 기존 VacationData에 필요한 속성 추가
export interface VacationTableRow extends VacationData {
  formattedPeriod: string;
  approverPosition?: string; // 스키마의 position 데이터를 받기 위해 추가
}
...
export const VacationTable = ({ data, onItemClick }: VacationTableProps) => {
  ...
  return (
    ...
    <div className="overflow-x-auto scrollbar-hide [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      {/* 💡 border-separate를 제거하여 조잡함을 없애고 밀착형 UI 적용 */}
        ...
        <thead>
          <tr className="border-b border-gray-100">
            {[
              ...
              <th
                key={idx}
                className="px-6 py-4 text-[13px] font-bold text-[#A3AED0] uppercase tracking-wider text-left first:pl-10"
              >
                {head}
              </th>
            ))}
          ...
          {data.length === 0 ? (
            ...
            data.map((item) => (
              <tr
                key={item.id}
                onClick={() => onItemClick(item)}
                /* 💡 붕 뜨는 그림자 대신 얇은 구분선과 호버 배경색만 적용 */
              ...
              >
                ...
                {/* 승인자 (이름 + 직급 옆에 나란히) */}
                <td className="px-6 py-5 text-left last:pr-10">
